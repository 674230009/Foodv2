...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
        ...
        // ลบวัตถุดิบเก่าออกก่อนเพื่อบันทึกใหม่ยกเซ็ต ป้องกันข้อมูลซ้ำซ้อน
        $pdo->prepare("DELETE FROM recipes WHERE food_id = ?")->execute([$id]);
        $food_id = $id;
    } else {
        // --- ส่วนเพิ่มใหม่ (Create) ---
        $stmt = $pdo->prepare("INSERT INTO foods (name_th, category) VALUES (?, ?)");
        $stmt->execute([$name_th, $category]);
        $food_id = $pdo->lastInsertId();
    }

    // บันทึกข้อมูลวัตถุดิบ (Recipes List) จากอาเรย์ที่ส่งมาจากฟอร์ม
